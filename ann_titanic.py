...
for layer in range(hidden_layer_number):
    hidden_layer_size = st.number_input("Enter The Number of Neurons in Hidden Layer " + str(layer+1), 1, 100, 3, 1)
    hidden_layer_sizes.append(hidden_layer_size)



activation = st.selectbox("Select The Activation Function", [
                          "identity", "logistic", "tanh", "relu"])
solver = st.selectbox("Select The Solver", ["lbfgs", "sgd", "adam"])
alpha = st.number_input("Enter The Alpha Value", 0.0001, 0.1, 0.01, 0.0001, format="%.4f")
learning_rate = st.selectbox("Select The Learning Rate", [
                             "constant", "invscaling", "adaptive"])
max_iter = st.number_input("Enter The Maximum Iterations", 100, 1000, 300, 100)

aiot4 = MLPClassifier(hidden_layer_sizes=hidden_layer_sizes, activation=activation,
                              solver=solver, alpha=alpha, learning_rate=learning_rate, max_iter=max_iter)
...
if train_file is not None:
    ...
    # add a button to create trainX and trainY
        ...
        with st.spinner("Training the model..."):
            ...
            st.success("Model Score: {:.3f}".format(
                aiot4.score(trainX, trainY)))
